...
Alice `writeFile("config.ts", ...)`。Bob 也 `writeFile("config.ts", ...)`。两个人改同一个文件，互相覆盖。而且无法干净地回滚——分不清哪些改动是谁的。
...
| 能力 | 作用 |
|------|------|
| createWorktree | 为任务创建独立目录 + 独立分支 |
| bindTaskToWorktree | 把任务和工作目录绑定（不改状态） |
| removeWorktree / keepWorktree | 完成后清理或保留 |
...
教学版给每个队友维护一个 `worktreeContext` 字典，记录当前 worktree 路径。队友认领带 worktree 的任务时，`worktreeContext` 自动设置为 worktree 路径；队友的 `bash`、`readFile`、`writeFile` 在 worktree 目录下执行：
...
Keep = 留着分支，等人工 review 后合并到主分支。Remove = 有改动时默认拒绝，需要 `discardChanges=true` 确认。不自动 complete task——任务完成由队友的 `completeTask` 显式触发。
...
`createWorktree` 和 `removeWorktree` 只在 git 命令成功后才写事件日志，保证日志反映真实状态。
...
| 新函数 | — | createWorktree, bindTaskToWorktree, removeWorktree, keepWorktree, validateWorktreeName |
| worktree 安全 | 无 | name 校验 + 有改动时拒绝删除 |
| 事件日志 | 无 | events.jsonl 生命周期审计 |
| Lead 工具 | 14 (s17) | + createWorktree, removeWorktree, keepWorktree (17) |
...
`ExitWorktreeTool.ts:261-320` 的 keep/remove 都会 `restoreSessionToOriginalCwd()` 恢复原目录。Remove 时检查未提交改动（`ExitWorktreeTool.ts:190-220`），没有 `discardChanges: true` 就拒绝删除。
...
`worktree.ts:76-84` 校验 slug：拒绝 `.`/`..`，允许 `[a-zA-Z0-9._-]`。`worktree.ts:48` 定义 `VALID_WORKTREE_SLUG_SEGMENT`。教学版的 `validateWorktreeName` 用同样的规则。
