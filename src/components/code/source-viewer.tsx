...
function highlightLine(line: string): React.ReactNode[] {
  ...
  const keywordSet = new Set([
    "type", "interface", "class", "import", "from", "return", "if", "else",
    "while", "for", "of", "in", "const", "let", "var", "new", "true",
    "false", "try", "catch", "throw", "with", "as", "yield", "break",
    "continue", "async", "await", "private", "readonly", "constructor",
  ]);

  const parts = line.split(
    /(\b(?:type|interface|class|import|from|return|if|else|while|for|of|in|const|let|var|new|true|false|try|catch|throw|with|as|yield|break|continue|async|await|private|readonly|constructor|this)\b|`(?:[^`\\]|\\.)*`|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|\/\/.*$|\b\d+(?:\.\d+)?\b)/
  );
